...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Z-score anomaly analysis on annual financial features")
    ...
    anomalies_df = build_zscore_anomalies_long(
        scored_df=scored_df,
    ...
    print("\nSelected features for Z-score:")
    print(feature_report_df[feature_report_df["selected"]].to_string(index=False))

    print("\nExcluded features:")
    excluded_df = feature_report_df[~feature_report_df["selected"]]
